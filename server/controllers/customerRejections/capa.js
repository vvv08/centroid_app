  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const [issues] = await db.query(
        `select u.description as uom , r.cust_rej_id, i.invoice_number, r.description as problem,  date_format(CONVERT_TZ(r.created_date, '+00:00', '+05:30'), '%Y-%m-%d') as created_date, date_format(CONVERT_TZ(r.last_updated, '+00:00', '+05:30'), '%Y-%m-%d') as last_updated , r.rejected_qty from cust_rejection_issues r inner join invoices i on r.invoice_id = i.invoice_id inner join unit_of_measurements u on r.uom_id = u.uom_id where r.invoice_id = ${id};`
      );

      const [containment] = await db.query(
        `select  date_format(CONVERT_TZ(c.created_date, '+00:00', '+05:30'), '%Y-%m-%d') as created_date, date_format(CONVERT_TZ(c.last_updated, '+00:00', '+05:30'), '%Y-%m-%d') as last_updated  ,c.containment_id, c.stock_check_supplier, date_format(c.supplier_date, '%Y-%m-%d') as supplier_date, c.stock_check_customer , date_format(c.customer_date, '%Y-%m-%d') as customer_date , c.stock_check_production , date_format(c.production_date, '%Y-%m-%d') as production_date, c.stock_check_transit , date_format(c.transit_date, '%Y-%m-%d') as transit_date, c.remarks , r.description as problem from cust_containment_actions c inner join cust_rejection_issues r on c.cust_rej_id = r.cust_rej_id where c.cust_rej_id in (select cust_rej_id from cust_rejection_issues where invoice_id = ${id} group by cust_rej_id);`
      );

      const [root_causes] = await db.query(
        `select rt.root_id, rt.description as root_cause, rt.remarks ,  date_format(CONVERT_TZ(rt.created_date, '+00:00', '+05:30'), '%Y-%m-%d') as created_date, date_format(CONVERT_TZ(rt.last_updated, '+00:00', '+05:30'), '%Y-%m-%d') as last_updated  , r.description as problem from cust_root_causes rt inner join cust_rejection_issues r on rt.cust_rej_id = r.cust_rej_id where rt.cust_rej_id in (select cust_rej_id from cust_rejection_issues where invoice_id = ${id} group by cust_rej_id);`
      );

      const [corrective_actions] = await db.query(
        `select c.corrective_id , c.description as corrective_action ,  date_format(CONVERT_TZ(c.created_date, '+00:00', '+05:30'), '%Y-%m-%d') as created_date, date_format(CONVERT_TZ(c.last_updated, '+00:00', '+05:30'), '%Y-%m-%d') as last_updated  , c.remarks , i.name as inspector , rt.description as problem from cust_corrective_actions c inner join cust_rejection_issues rt on c.cust_rej_id = rt.cust_rej_id inner join inspectors i on c.inspector_id = i.inspector_id where c.cust_rej_id in (select cust_rej_id from cust_rejection_issues where invoice_id = ${id} group by cust_rej_id);`
      );

      const [preventive_actions] = await db.query(
        `select p.preventive_id , p.description as preventive_action ,  date_format(CONVERT_TZ(p.created_date, '+00:00', '+05:30'), '%Y-%m-%d') as created_date, date_format(CONVERT_TZ(p.last_updated, '+00:00', '+05:30'), '%Y-%m-%d') as last_updated  , p.remarks , i.name as inspector , r.description as problem from cust_preventive_actions p inner join cust_rejection_issues r on p.cust_rej_id = r.cust_rej_id inner join inspectors i on p.inspector_id = i.inspector_id where p.cust_rej_id in (select cust_rej_id from cust_rejection_issues where invoice_id = ${id} group by cust_rej_id);`
      );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const [result] = await db.query(
        `insert into cust_rejection_issues (invoice_id,description,rejected_qty,uom_id,created_date,last_updated) values (${invoice},"${description}",${rejected_qty},${uom},NOW(),NOW());`
      );
      let insertedIssue = {
        id: result.insertId,
        invoice,
...
export const editIssue = ({ cust_rej_id, description, rejected_qty, uom }) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const [result] = await db.query(
        `update cust_rejection_issues set description = "${description}" , rejected_qty = ${rejected_qty},uom_id = ${uom} ,last_updated = NOW() where cust_rej_id = ${cust_rej_id};`
      );
      let editedIssue = {
        cust_rej_id,
        description,
...
export const editContainment = ({
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const [result] = await db.query(
        `update cust_containment_actions set stock_check_supplier = "${stock_check_supplier}", supplier_date = "${supplier_date}", stock_check_customer = "${stock_check_customer}", customer_date = "${customer_date}", stock_check_production = "${stock_check_production}", production_date = "${production_date}", stock_check_transit = "${stock_check_transit}", transit_date = "${transit_date}", remarks = "${remarks}", last_updated = NOW() where containment_id = ${containment_id};`
      );
...
export const addCorrective = ({ issue, inspector, description, remarks }) => {
  return new Promise(async (resolve, reject) => {
    try {
      const [result] = await db.query(
        `insert into cust_corrective_actions (description,inspector_id,cust_rej_id,remarks,created_date,last_updated) values ("${description}",${inspector},${issue},"${remarks}",NOW(),NOW());`
      );
...
export const editCorrective = ({
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const [result] = await db.query(
        `update cust_corrective_actions set inspector_id = ${inspector}, description = "${description}", remarks = "${remarks}", last_updated  = NOW() where corrective_id = ${corrective_id}`
      );
...
export const addRootCause = ({ issue, description, remarks }) => {
  return new Promise(async (resolve, reject) => {
    try {
      const [result] = await db.query(
        `insert into cust_root_causes (cust_rej_id,description,remarks,created_date,last_updated) values (${issue},"${description}","${remarks}",NOW(),NOW());`
      );
...
export const editRootCause = ({ root_id, description, remarks }) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const [result] = await db.query(
        `update cust_root_causes set description = "${description}", remarks = "${remarks}", last_updated = NOW() where root_id = ${root_id};`
      );
      let editedRootCause = {
        root_id,
        description,
...
export const addPreventiveAction = ({
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const [result] = await db.query(
        `insert into cust_preventive_actions (cust_rej_id,description,inspector_id,remarks,created_date,last_updated) values (${issue},"${description}",${inspector},"${remarks}",NOW(),NOW());`
      );
...
export const editPreventiveAction = ({
  ...
  return new Promise(async(resolve,reject) => {
    try{
      ...
      const [result] = await db.query(`update cust_preventive_actions set inspector_id = ${inspector}, description = "${description}",remarks = "${remarks}", last_updated = NOW() where preventive_id = ${preventive_id}`)
