...
//Add a issue
export const addIssue = ({ invoice, description, rejected_qty }) => {
  return new Promise(async (resolve, reject) => {
    try {
      const [result] = await db.query(
        `insert into cust_rejection_issues (invoice_id,description,rejected_qty,created_date,last_updated) values (${invoice},"${description}",${rejected_qty},"${curr_date}","${curr_date}");`
      );
      let insertedIssue = {
        id: result.insertId,
        invoice,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const [[issue]] = await db.query(
        `select cust_rej_id, description as problem, rejected_qty from cust_rejection_issues where cust_rej_id = ${cust_rej_id};`
      );
      resolve(issue);
    } catch (err) {
      reject(err);
...
//To edit a issue
export const editIssue = ({ cust_rej_id, description, rejected_qty }) => {
  return new Promise(async (resolve, reject) => {
    try {
      const [[issue]] = await db.query(
      ...
      const [result] = await db.query(
        `update cust_rejection_issues set description = "${description}" , rejected_qty = ${rejected_qty}, last_updated = "${curr_date}" where cust_rej_id = ${cust_rej_id};`
      );
      let editedIssue = {
        cust_rej_id,
        description,
